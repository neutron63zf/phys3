...
/* calculate v1 + a * v2 */
double* linear(int n, double *v1, double a, double *v2) {
...
/* calculate y = A * x */
double* matmul(int n, double **a, double *x) {
  int i, j;
  double *y = alloc_dvector(n);
  for (i = 0; i < n; ++i) {
    y[i] = 0;
    for (j = 0; j < n; ++j) {
      y[i] += mat_elem(a, i, j) * x[j];
    }
  }
  return y;
}
...
int main(int argc, char** argv) {
  ...
  for (k = 0; k < 1000; ++k) {
    alpha = inner(n, r, p) / quad(n, a, p);
    x = linear(n, x, alpha, p);
    residual(n, a, b, x, r);
    beta = - inner(n, p, matmul(n, a, r)) / quad(n, a, p);
    p = linear(n, r, beta, p);
